...
abstract class ForageableRoomDatabase : RoomDatabase() {

    abstract fun ForageableDao(): ForageableDao

    companion object {
        @Volatile
        private var INSTANCE: ForageableRoomDatabase? = null

        fun getDatabase(context: Context): ForageableRoomDatabase {
            // if the INSTANCE is not null, then return it,
            // if it is, then create the database
            return INSTANCE ?: synchronized(this) {
                val instance = Room.databaseBuilder(
                    context.applicationContext,
                    ForageableRoomDatabase::class.java,
                    "forageable_database"
                )
